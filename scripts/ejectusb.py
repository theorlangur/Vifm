...
targetDrive = None
if len(sys.argv) > 1:
    targetDrive=sys.argv[1].upper()[0]
mydrives=checkRemovableDrives()
if mydrives:
    #print(mydrives)
    for drive in mydrives:
        if not targetDrive or targetDrive == drive[0].upper():
            ...
            if targetDrive:
                exit(0)
    if targetDrive:
        print("Ejectable drive {0} not found".format(targetDrive), file=sys.stderr)
else:
    print("No removable drives", file=sys.stderr)
